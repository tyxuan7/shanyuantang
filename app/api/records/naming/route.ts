...
export async function POST(request: NextRequest) {
  try {
    const { surname, gender, birth_date, style, result_text } = await request.json();
    ...
    const id = uuidv4();
    await sql("INSERT INTO naming_records (id,user_id,surname,gender,birth_date,style,result_text,created_at) VALUES (?,?,?,?,?,?,?,?)",
      [id, userId, surname||"", gender||"unknown", birth_date||"", style||"", result_text||"", now()]);
